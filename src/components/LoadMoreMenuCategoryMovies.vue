...
export default {
  ...
  computed: {
    currentPage: {
      get() {
        return this.$store.state.currentMenuCatPage;
      },
      set(value) {
        return (this.$store.state.currentMenuCatPage = value);
      }
    },
    APIKEY() {
  ...
  methods: {
    ...
    fetchMoreMovies: async function() {
      ...
      this.$store.dispatch("fetchMoreMenuCategoryMovies", movies.results);
      this.isBussy = false;
